...
\section*{Abstract}
Fitting multiple figures into very tight manuscripts while keeping it pleasant to read is challenging. Therefore figures are often simply attached to the very end of a manuscript. While being easy for the authors, this practice is really inconvenient for readers. This \LaTeX template shows how to generate a compiled PDF with figures embedded into the text. It provides several examples of how to embed figures or tables directly into the text. This is supposed to give you a range of options from which you should choose the one best suited for your data. Check out Schlegel et al., (2016) as example of use \cite{Schlegel2016}.
...
In the introduction you will see an example of text wrapping around the figure with a figure legend on the margin (Fig. \ref{fig1}). This is done by combining the \verb!wrapfigure! with % avoid blank space here 
\marginpar{\vspace{.7cm} \color{Gray} % adjust vertical position relative to text with \vspace{} - note that you can enter negative numbers to move margin caption up
\textbf{Figure \ref{fig1}. Example of a margin caption.} % note that \ref{fig1} refers to the embedded wrapfigure in the text
\textbf{A}, Setting up your figure and legend like this looks fancy and does not disrupt the flow of the text. \textbf{B}, But it requires more manual adjustments (position, spacing, labeling) than if you used standard \LaTeX figure environments.
}
